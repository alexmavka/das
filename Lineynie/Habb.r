main <- function(objectCounter = 200){
  ...
  plot(xl[,1],xl[,2], pch = 21,main = "Habb", xlab = 'feature 1', ylab= 'feature 2', col = colors[xl[,4]], asp = 1, bg=colors[xl[,4]])
...
4stochGrad <- function(xl, teta, lamda){
  new_classes <- xl[,4]
  for(i in 1 : dim(xl)[1]){
    if(new_classes[i] == 1){
      new_classes[i] = -1
  ...
  while(TRUE){
    ...
    if((crossprod(x_rand[1:3],w) * new_classes[ind]) < 0){
      w <- w + teta*x_rand[1:3]*new_classes[ind]
